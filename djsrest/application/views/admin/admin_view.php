<div class="col-md-9">
	<h2>Administrator</h2>
	<hr>
	<a href='<?php echo site_url('admin/user_management')?>'>User Management</a><br>
	<a href='<?php echo site_url('admin/employee_management')?>'>Employee Management</a><br>
	<a href='<?php echo site_url('admin/customer_management')?>'>Customer Management</a><br>	
	<a href='<?php echo site_url('admin/Reservation_management')?>'>Reservation Management</a><br>
	<a href='<?php echo site_url('admin/credit_card_management')?>'>Credit card Management</a><br>
	<a href='<?php echo site_url('admin/user_group_management')?>'>User Group Management</a><br>
</div>